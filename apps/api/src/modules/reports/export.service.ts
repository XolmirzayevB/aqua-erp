...
export class ExportService {
  ...
  async toExcel(query: ReportQueryDto): Promise<Buffer> {
    ...
    const rows = [
      ...
      ["Kutilayotgan Klik (so'm)", overview.finance.pendingClick],
      ["Yozilgan qarzlar (so'm)", overview.finance.debtsWritten],
  ...
  async toPdf(query: ReportQueryDto): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const summaryLines = [
        ...
        `Kutilayotgan Klik: ${overview.finance.pendingClick.toLocaleString()} so'm`,
        `Yozilgan qarzlar: ${overview.finance.debtsWritten.toLocaleString()} so'm`,
